...
Template.Table.onCreated(function(){
    ...
    let sort = dot(this,'data.sort') ? dot(this,'data.sort') : (dot(this,'data.sorting') ? dot(this,'data.sorting') : {name: 1});
    this.startWithFirstPage = dot(this,'data.firstPageStart') ? dot(this,'data.firstPageStart') : false;
    ...
    this.uuid =  dot(this,'data.uuid') || new ReactiveVar((dot(this,'data.collection._name') + ':' + _.pluck(dot(this,'data.columns'),'label').join('|')));
    ...
    this.autorun(() => {
        ...
        _.each(dot(this,'data.filters'),(filter) => {
    ...
    this.autorun(() => {
        ...
        try {
            if (EJSON.parse(localStorage.getItem('Table.' + uuid))) {
                ...
                if (settings.page) {
                    if(this.startWithFirstPage === false){
                        this.page.set(settings.page);
                    }
                }

                if (settings.limit) {
                    this.limit.set(settings.limit);
                }
    ...
    // obsługa subskrypcji
    this.autorun(() => {
        let selector = dot(this,'data.selector') ? _.clone(dot(this,'data.selector')) : {};

        let fields = dot(this,'data.fields') ? dot(this,'data.fields') : {};

        _.extend(selector,this.filterSelector.get());
        _.extend(selector,this.searchSelector.get());

        this.subscribe(dot(this,'data.collection._name'),
            selector,
            {
                sort: this.sort.get(),
...
Template.Table.helpers({
    ...
    rows() {
        ...
        let selector = dot(Template.instance(),'data.selector') ? _.clone(Template.instance().data.selector) : {};
        ...
        return dot(t,'data.collection') ? t.data.collection.find(selector,{ sort: t.sort.get(), limit: t.limit.get()}) : null;
    },
    getColumnValue() {
    ...
    getColumnData() {
        let data = Template.parentData(1);
        if(typeof dot(this,'data.call') === 'function') return this.data.call(data);
    },
    searchQuery() {
    ...
    getRowAttributes() {
        if (_.isFunction(Template.instance().data.getRowAttributes)) {
            return Template.instance().data.getRowAttributes.call(this,Template.instance().data);
        }
    }
});
...
Template.Table.events({
    ...
    'click .TableAction' (e,t) {
        e.preventDefault();
        if (_.isFunction(this.onClick)) {
            this.onClick.call(this,Template.currentData(),Template.instance());
        }
    },
    'click tbody tr' (e,t) {
        e.preventDefault();
